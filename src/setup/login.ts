...
export async function runLogin(): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    const server = createServer({ key: tls.key, cert: tls.cert }, (request, response) => {
      ...
      exchangeCode(code)
        .then(() => {
          ...
          response.end(
            reply("<h2>Authenticated ✨</h2><p>You can close this tab and return to Claude.</p>"),
          );
          server.close();
          resolve();
        })
    ...
    server.listen(CALLBACK_PORT, "127.0.0.1", () => {
      console.error("[destiny2-mcp] Opening browser to log in to Bungie...");
      console.error(`[destiny2-mcp] If it doesn't open, visit:\n${authorizeUrl}`);
      if (!tls.trusted) {
        console.error(
