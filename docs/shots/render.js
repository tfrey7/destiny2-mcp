...
(function () {
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  const M = CFG.margin ?? 380, T = CFG.top ?? 82, B = CFG.bottom ?? 56;
  const stage = document.createElement('div'); stage.id = 'stage'; document.body.appendChild(stage);
  const shot = document.createElement('img'); shot.id = 'shot'; stage.appendChild(shot);
  const svg = document.createElementNS('http://www.w3.org/2000/svg','svg'); svg.id='lines'; stage.appendChild(svg);

  shot.onload = () => {
    const W = shot.naturalWidth, H = shot.naturalHeight;
    const pageW = W + 2*M, pageH = H + T + B;
    stage.style.width = pageW+'px'; stage.style.height = pageH+'px';
    shot.style.left = M+'px'; shot.style.top = T+'px';

    const ttl = document.createElement('div'); ttl.id='title'; ttl.textContent = CFG.title;
    ttl.style.left = M+'px'; ttl.style.top = '28px'; stage.appendChild(ttl);
    const sub = document.createElement('div'); sub.id='subtitle'; sub.textContent = CFG.subtitle;
    sub.style.left = M+'px'; sub.style.top = '52px'; stage.appendChild(sub);
    const key = document.createElement('div'); key.className='key';
    key.innerHTML = '<b>blue = static manifest definition</b> &nbsp;·&nbsp; <i>gold = live per-instance / live profile data</i>';
    key.style.left = M+'px'; key.style.top = (pageH-22)+'px'; stage.appendChild(key);

    svg.setAttribute('width', pageW); svg.setAttribute('height', pageH);
    const anchors = [];
    CFG.ann.forEach((a) => {
      const d = document.createElement('div'); d.className = 'lbl '+a.side;
      ...
      stage.appendChild(d); anchors.push({d,a,w});
    });
    requestAnimationFrame(() => {
      ...
      anchors.forEach(({d,a,w}) => {
        ...
        const tx = M + a.t[0], ty = T + a.t[1];
        const col = a.y ? '#caa23a' : '#3f7fb5', dot = a.y ? '#ffd76b' : '#7fd0ff';
      ...
      svg.innerHTML = s; document.title = 'READY';
    });
  };
  shot.src = CFG.img;
})();
